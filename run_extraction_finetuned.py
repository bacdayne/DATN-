...
OUTPUT_FILE    = os.path.join(OUTPUT_DIR, "predictions_test_finetuned.json")
FINETUNED_MODEL = "finetuning/qwen2-cord-merged"
...
def fix_to_schema(data):
    ...
    sub   = data.get("sub_total", {}) if isinstance(data.get("sub_total"), dict) else {}
...
def main():
    ...
    for item in ocr_results:
        ...
        prompt     = get_prompt_by_agent(agent_name=agent_used, ocr_text=ocr_text)
        ...
        if not ok:
            ...
            prediction_text, raw_prediction, ok = generate_prediction(
                llm, sampling_params, retry_prompt
            )
